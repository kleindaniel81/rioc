...
{pstd}
{cmd:rioc} 
calculates the relative improvement over chance (RIOC) coefficient 
for 2 x 2 tables (Farrington & Loeber 1989, Copas & Loeber 1990). 
The RIOC corrects the observed proportion of correctly classified subjects 
for chance and maximum ceiling.

{pstd}
The command is implemented using methods and formulas 
discussed in Copas and Loeber (1990) and in Farrington and Loeber (1989). 
In both variables {it:prediction} and {it:outcome}, 
values equal to nonzero and nonmissing (typically values of one) 
indicate a true (positive) state; 
values equal to zero indicate a false (negative) state.


{title:Remarks}

{pstd}
Although the syntax diagram implies 
that the order of the variables is always relevant, 
the relative improvement over chance (RIOC) 
does not depend on the orientation of the contingency table. 
However, the orientation of the contingency table 
does affect other available statistics, 
such as the sensitivity and specificity.
...
This is the default for the immediate command, {cmd:rioci}; 
specify {opt notab} to suppress the contingency table.

{phang}
{opt detail} 
calculates the total (observed), chance (expected), 
and maximum possible proportion of correctly classified subjects. 
Confidence intervals are calculated using Stata's {helpb ci:cii} command 
with the default exact method.

{phang}
{opt stats(stat)} 
requests additional proportions. 
Confidence intervals are calculated using Stata's {helpb ci:cii} 
with the default exact method; 
...
{phang}
{cmd:stats(}{it:name}{cmd::} {it:numerator}{cmd:/}{it:denominator}{cmd:)} 
is an alternative syntax for {opt stats()} 
that defines proportions to be calculated. 
Confidence intervals are calculated using Stata's {helpb ci:cii} 
with the default exact method. 
{it:name} is a valid name for the proportion, 
and {it:numerator} and {it:denominator} are {it:{help expression:expressions}} 
that typically refer to the cells of the contingency table. 
For example, in {it:numerator} and {it:denominator}, 
{cmd:a} refers to the number of true positives 
and {cmd:n} refers to the total number of subjects. 
The notation follows Copas and Loeber (1990):
...
{phang2}{...}
More than one {it:stat} may be defined. 
For example, sensitivity (the true positive rate) 
and specificity (the true negative rate) 
may be obtained as 
...
{phang2}{...}
Note that both {it:numerator} and {it:denominator} 
must be enclosed in parentheses if they contain spaces. 
More technically, the defined {it:stats} consist out of 5 tokens: 
{it:name}, {cmd::}, {it:numerator}, {cmd:/}, and {it:denominator}.

{phang2}
Although, technically, {cmd:rioc} can compute risk-ratios, 
e.g., {cmd: stats(LRpos: (a*(n-f))/(f*b))}, 
the confidence intervals are still calculated using {helpb ci:cii}; 
for risk-ratios, confidence intervals are not valid (see {helpb cs:csi}).

{phang}
{opt kappa} 
calculates Cohen's kappa. 
The standard error and the confidence interval are implemented 
according to Fleiss et al. (1969). 
Option {opt sezero} may be specified 
to obtain the same standard error as Stata's {helpb kap} command. 

{phang}
{opt sezero} 
calculates the standard error of RIOC 
(and kappa, if {opt kappa} is also specified) 
for testing against zero. 
For the RIOC, the p-value 
matches the p-value of Pearson's chi-squared test with 1 degree of freedom; 
for kappa, the standard error and z-value 
match those of Stata's {helpb kap} commmand. 
Note that confidence intervals are not affected by {opt sezero}.

{phang}
{opt smallsample} 
calculates the confidence interval for RIOC in small samples. 
{opt smallsample} only affects the confidence interval for the RIOC; 
no confidence intervals are calculated 
for additional proportions and statistics. 
Also, when {opt smallsample} is specified, 
...
The default is {cmd:level({ccl level})} (see {helpb level:c(level)}).

{marker opt_di}{...}
{phang}
{cmd:cformat(}{it:{help format:{bf:%}fmt}}{cmd:)} 
specifies how to format coefficients, standard errors, and confidence limits. 
The maximum format width is 9.

{phang}
{cmd:pformat(}{it:{help format:{bf:%}fmt}}{cmd:)} 
specifies how to format p-values. 
The maximum format width is 5.

{phang}
{cmd:sformat(}{it:{help format:{bf:%}fmt}}{cmd:)} 
specifies how to format test statistics. 
The maximum format width is 8.
...
{pstd}
Example 1a: Keep the dataset in memory, suppress table (Caution: clears the dataset)

{phang2}{stata rioci 75 125 \ 25 175 , replace notab:. rioci 75 125 \ 25 175 , replace notab}{p_end}

{pstd}
Example 2: More details

{phang2}{stata rioc prediction outcome [fweight=pop] , detail:. rioc prediction outcome [fweight=pop] , detail}{p_end}

{pstd}
Example 3: Add Sensitivity and Specificity, display contingency table
...
{psee}
if installed: {help classtabi}, {help diagt}, {help kappaetc}, {help loevh2}
{p_end}
