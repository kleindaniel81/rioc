...
program rioc , rclass byable(recall)
    ...
    /*
        ...
        Note: We flip the coding so that zero indicates 'true'
              and nonzero and nonmissing indicates 'false' 
              to replicate the tables in Copas and Loeber (1990) 
    */
    ...
    Binarize `prediction' `outcome' `touse' `varlist' `plbl' `olbl' , `tab'    
    ...
    /*
        Copas and Loeber (1990:303) present all formulas assuming that e >= f; 
        "if not, rows and columns are interchanged"
    */
...
program Detail
    ...
    if (`"`stats'"' != "") {
        ...
        Parse_stats `a' `b' `c' `d' `e' `f' `n' , `stats' `detail'
...
program Parse_stats
    ...
    while (`"`stats'"' != "") {
        ...
        gettoken numer stats : stats , parse(" /") match(junk)
        ...
        if (`"`semi'"'!=";") ///
            local stats `semi' `stats'
        
        c_local STAT`++i' `denom' `numer'
...
program error_stats
    ...
    display as err "above applies to option {bf:stats()}"
    exit 198
...
program Kappa
    ...
    if ("`smallsample'" != "smallsample") {
        ...
        if ( ("`asr'"=="asr") | ("`chi2'"=="chi2") ) ///
            ...
            matrix `tc' = (`kappa'\ `s'\ `z'\ `p'\ `ll'\ `ul'\ `crit'\ .\ 0)
...
program Set_fmt
    ...
    if (fmtwidth(`"`ww'"') <= fmtwidth("`w'")) ///
        c_local `f' `ww'
    else ///
        display as txt "note: invalid `f'ormat(), using default"
...
2.0.4   16jul2026   code polish and revised comments
2.0.3   16may2025   code polished
2.0.2   23mar2021   more typos in comments
...
1.0.0   26jan2021   new options -detail-, -stats()-, -{c s p}format()-
                    ...
                    new help files
0.0.9   23jan2021   posted to Statalist
